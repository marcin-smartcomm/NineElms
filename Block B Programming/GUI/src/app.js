...
function ShowRoomChangeIcon(neighbourRoomID)
{
    $.get("components/room-change-btn/room-change-btn.html", (data) => 
    { 
        $('#roomNameContainer').append(data)
        $('#roomIDX').attr('id', `roomID${neighbourRoomID}`)
        
        $(`#roomID${neighbourRoomID}`).on('touchend', () => {
            let callResult = AjaxGETCall("ChangeRoom", [neighbourRoomID])
